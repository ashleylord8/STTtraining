...
class Commands {
  ...
  createAndTrainCustomization(params) {
    ...
    .then((customization) =>
      ...
      .then((newCustomization) =>
        buildCorpusFromWorkspace(customization.workspace)
        .then(corpusFromWorkspace => {
          const newCorpus = Object.assign(corpusFromWorkspace, newCustomization);
          return this.speechService.createCorpusAsync(newCorpus);
        })
        .then(() =>
          this.speechService.whenCorporaAnalyzed(newCustomization)
        )
        .then(() =>
          this.speechService.trainCustomization(newCustomization)
        )
      )
    );
  }
...
function promptForCreateCustomizationParameters(service, _params) {
  ...
  if (typeof(_params.workspace) === 'string') {
    _params.workspace = files.readJSONFile(_params.workspace);
  }
