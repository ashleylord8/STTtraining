    ...
    getExamples(params, function(error) {
        if (error){
            ...
            return new Promise((resolve, reject) => {
                speech_to_text.createCustomization(params)
                    .then((customization_id) => {
                        console.log('2. Model customization_id: ', customization_id, '\n');
                        fs.readFile(params.examples, function(error, data) {
                            if (error) {
                                callback(error, null);
                            } else {
                                return speech_to_text.addCorpus({
                                        'customization_id': customization_id,
                                        'corpus': data,
                                        'name': 'corpus1'
                                    })
                                    .then((addCorpusResponse) => {
                                        ...
                                        return speech_to_text.whenCorporaAnalyzed({
                                                'customization_id': addCorpusResponse.customization_id
                                            })
                                            .then((corporaAnalysisResponse) => {
                                                console.log('4.2 Corpus status: ', corporaAnalysisResponse.status);
                                                console.log('4.3 Corpus analysis done!\n');
                                                return speech_to_text.getWords(corporaAnalysisResponse)
                                                    .then((getWordsResponse) => {
                                                        ...
                                                        return speech_to_text.whenCustomizationReady(getWordsResponse)
                                                            .then((customizationStatusResponse) => {
                                                                console.log('6.2 Model status: ', customizationStatusResponse.status);
                                                                console.log('6.3 Model is ready to be trained!\n');
                                                                return speech_to_text.getWords(customizationStatusResponse)
                                                                    .then((getWordsResponse) => {
                                                                        console.log('7. Below is the list of Out of Vocabulary words ->');
                                                                        console.log(getWordsResponse.words, '\n');
                                                                        return speech_to_text.trainCustomization(getWordsResponse)
                                                                            .then((trainingResponse) => {
                                                                                ...
                                                                                return speech_to_text.whenCustomizationReady(trainingResponse)
                                                                                    .then((customizationStatusResponse) => {
...
* @type {Object} params customization id
*/
var list_customizations = function(params, callback) {
  const speech_to_text = require('./stt.js')(params);
  return new Promise((resolve, reject) => {
    speech_to_text.getCustomizations(params)
    .then((getCustomizationsResponse)=>{
      console.log('\nBelow is the list of all customizations ->');
      console.log(getCustomizationsResponse.customizations, '\n');
    })
    .catch((error) => {
        console.log('ERROR: Could not retrieve the list of customizations');
        console.log(error);
        reject(error);
    });
  });
...
var delete_customization = function(params, callback) {
  const speech_to_text = require('./stt.js')(params);
  return new Promise((resolve, reject) => {
    speech_to_text.deleteCustomization(params)
    .then((deleteCustomizationResponse)=>{
      console.log('\nModel deletion returns: ', deleteCustomizationResponse.status);
    })
    .catch((error) => {
        console.log('ERROR: Could not delete the customization');
        console.log(error);
        reject(error);
    });
  });
...
var corpus_status = function(params, callback) {
  const speech_to_text = require('./stt.js')(params);
  return new Promise((resolve, reject) => {
    console.log('\n1.1 Checking status of corpus... Please Wait...');
    speech_to_text.getCorpora(params)
    .then((getCorpusResponse)=>{
      console.log('1.2 Corpus status: ', getCorpusResponse.status);
      if (getCorpusResponse.status == 'analyzed') {
        console.log('1.3 Corpus analysis done!');
      } else if (getCorpusResponse.status == 'being_processed') {
        console.log('1.3 Corpus is still being processed!');
      }
    })
    .catch((error) => {
        console.log('ERROR: Could not retrieve the model status');
        console.log(error);
        reject(error);
    });
  });
...
var model_status = function(params, callback) {
  const speech_to_text = require('./stt.js')(params);
  return new Promise((resolve, reject) => {
    console.log('\n1.1 Checking status of model... Please Wait...');
    speech_to_text.getCustomization(params)
    .then((getCustomizationResponse)=>{
      console.log('1.2 Model status: ', getCustomizationResponse.status);
      if (getCustomizationResponse.status == 'ready') {
        console.log('1.3 Model is ready to be trained !');
      } else if (getCustomizationResponse.status == 'available') {
        console.log('1.3 Model has been trained !');
      }
    })
    .catch((error) => {
        console.log('ERROR: Could not retrieve the model status');
...
/**
* Show all out of vocbulary words
* @type {Object} params customization id
*/
var show_oovs = function(params, callback) {
    const speech_to_text = require('./stt.js')(params);
    return new Promise((resolve, reject) => {
      speech_to_text.getWords(params)
      .then((getWordsResponse)=>{
        console.log('\nBelow is the list of Out of Vocabulary words ->');
        console.log(getWordsResponse.words, '\n');
      })
      .catch((error) => {
          console.log('ERROR: Could not retrieve the list of OOVs');
          console.log(error);
          reject(error);
      });
    });
...
var add_word = function(params, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((addWordResponse)=>{
      ...
      return speech_to_text.trainCustomization(addWordResponse)
      .then((trainingResponse)=>{
...
var add_words = function(params, callback) {
  ...
  return new Promise((resolve, reject) => {
    ...
    .then((addWordsResponse)=>{
      console.log('1.1 Adding multiple words returns: ', addWordsResponse.status, '\n');
      console.log('1.2 Checking status of model... Please wait...');
      return speech_to_text.whenCustomizationReady(addWordsResponse)
          .then((customizationStatusResponse) => {
              console.log('1.3 Model status: ', customizationStatusResponse.status);
